...
use sr_primitives::traits::{Bounded, SimpleArithmetic};
use support::{
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn transfer(origin, to: T::AccountId, kitty_id: T::KittyIndex) -> Result {
            ...
            if owned_kitties_count_to == T::KittyIndex::max_value() {
                return Err("Kitties count overflow");
            }

            let owned_kitties_count_sender = Self::owned_kitties_count(sender.clone());
            if owned_kitties_count_sender == T::KittyIndex::min_value() {
                return Err("Kitties count overflow");
            }

            let _ = Self::kitty(kitty_id).ok_or("No found this kitty")?;
            ensure!(<OwnedKitties<T>>::exists((sender.clone(), kitty_id)), "Not own this kitty");

            let kitty = Self::owned_kitties((sender.clone(), kitty_id));

            <OwnedKitties<T>>::remove((sender.clone(), owned_kitties_count_sender));
            <OwnedKittiesCount<T>>::insert(sender, owned_kitties_count_sender - 1.into());

            <OwnedKitties<T>>::insert((to.clone(), owned_kitties_count_to), kitty_id);
            <OwnedKittiesCount<T>>::insert(to, owned_kitties_count_to + 1.into());
